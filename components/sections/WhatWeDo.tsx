      ...
      <div className="section-padding">
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-12 lg:gap-20">
          ...
          <div className="flex-1 w-full">
            ...
            {/* Mobile: circle-center style cycler + tappable index */}
            <div className="lg:hidden w-full">

              {/* Circle-center display — mirrors what's shown inside the wheel */}
              <div className="relative flex flex-col items-center justify-center text-center mb-8"
                ...
                <div className="absolute inset-4 rounded-full"
                  style={{ border: "1px dashed rgba(245,230,208,0.07)" }} />
                {/* Active colored ring */}
                {svc && (
                ...
                <AnimatePresence mode="wait">
                  {svc ? (
                    ...
                    >
                      <p className="text-[8px] tracking-[0.35em] uppercase text-[#C9A84C] font-bold">{svc.stat}</p>
                      <h3 className="font-display text-[#F5E6D0] leading-tight"
                        style={{ fontSize: "clamp(1rem,4.5vw,1.3rem)" }}>
                        {svc.title}
                      </h3>
                      <p className="text-[#F5E6D0]/40 leading-snug text-[10px]">{svc.tagline}</p>
                    </motion.div>
                  ) : (
                    <motion.div key="idle" className="flex flex-col items-center gap-1"
                      initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                      <span className="font-display text-[#F5E6D0]/15 leading-none text-5xl">{ALL.length}</span>
                      <span className="font-display text-[#F5E6D0]/20 text-[10px] tracking-[0.3em]">SERVICES</span>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
              ...
              <div className="flex flex-col divide-y divide-[#F5E6D0]/[0.06]">
                {ALL.map((s, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="font-display text-[10px] tracking-[0.35em] shrink-0 w-7"
                        style={{ color: isActive ? "#C9A84C" : "rgba(245,230,208,0.25)" }}>
                        {String(i + 1).padStart(2, "0")}
                      </span>
                      <span className="font-display flex-1 leading-tight"
                        style={{
